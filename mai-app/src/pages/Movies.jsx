...
const GENRES = [...new Set(movies.map(m => m.genre))]; //ดึงประเภทหนังที่ไม่ซ้ำกัน
...
function Movies() {
  ...
  m.title.toLowerCase().split(' ').some(word => word.startsWith(q)) && (genre === 'all' || m.genre === genre)); //เเยกคำแล้วตรวจสอบที่ละค่า

  return (
    <div className="mx-auto max-w-5xl p-8">
      <h1 className="mb-6 text-2xl font-bold text-slate-800">หนังทั้งหมด</h1>
      <SearchBox query={query} setQuery={setQuery} />
      <GenreBox genre={genre} GenreChange={setGenre} genres={GENRES} />
